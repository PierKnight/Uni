...
{
    return posRow % (ROWS / 8) == 0 || (posRow + 1) % (ROWS / 8) == 0;
}

inline void fishUpdate(creature* matrix,int i,int j,bool border)
{
    int pos;
    ...
    int posRow = pos / COLS;
    bool test = !border ? (isBorder(posRow) || pos > V(i,j)) : (isBorder(posRow) && (i + 1) % (ROWS / 8) == 0 ? (posRow == i ? pos > V(i,j) : true) : false);
...
inline void updateWorld(creature* matrix,int fromRow,int toRow,bool isBorder)
{
    for(int i = fromRow;i < toRow;i++)
    {
        ...
        for(int j = 0;j < COLS;j++)
        {
            if(!matrix[V(i,j)].moved)
            {
                ...
                if(c.type == FISH)
                    fishUpdate(matrix,i,j,isBorder);
