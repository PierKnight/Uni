...
class Tavolo:
    ...
    def __init__(self):
        ...
        self.mangiateFilosofi = [0,0,0,0,0]
    ...
    def prendiLockSimultaneo(self,posizione):
        ...
        with self.lock:
            while( self.bacchetta[posizione].checkOccupata() or self.bacchetta[(posizione+1) % 5].checkOccupata() or self.mangiateFilosofi[posizione] > 5):
                self.cond.wait()
            self.bacchetta[posizione].prendiBacchetta()
            self.bacchetta[(posizione+1) % 5].prendiBacchetta()
            self.mangiateFilosofi[posizione] += 1
            if(self.mangiateFilosofi[(posizione - 1) % 5] > 0):
                self.mangiateFilosofi[(posizione - 1) % 5] -= 1
            if(self.mangiateFilosofi[(posizione + 1) % 5] > 0):
                self.mangiateFilosofi[(posizione + 1) % 5] -= 1
