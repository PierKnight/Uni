...
class PartecipanteSafe(Thread):
    ...
    def __init__(self,posti,display):
        super().__init__()
        self.posti = posti
        self.display = display
        self.offset = randrange(len(self.posti))

    def run(self):
        #sleep(randrange(1))
        for i in range(0,len(self.posti)):
            i = (self.offset + i) % len(self.posti)
            if self.posti[i].testaEoccupa():
                #print( "Sono il Thread %s. Occupo il posto %d" % ( self.getName(), i ) ,flush=True)
                ...
                for i in range(len(self.posti)):
                    if(self.posti[i].libero()):
                        ...
                        output += "#"
                self.display.buffer.put(output)
                return                
        self.display.buffer.put("Oh no ho perso %s" % self.name)
        self.display.buffer.put("s")
